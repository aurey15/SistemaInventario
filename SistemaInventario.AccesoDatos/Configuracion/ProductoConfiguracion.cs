...
namespace SistemaInventario.AccesoDatos.Configuracion
{
    public class ProductoConfiguracion : IEntityTypeConfiguration<Producto>
    {
        public void Configure(EntityTypeBuilder<Producto> builder)
        {
            ...
            /* Relaciones*/
            //Si es de UNO a MUCHOS [ HasOne(x => x.NameModelo).WithMany().HasForeignKey(x => x.NameForeingKeyRelacionado); ]
            //
            builder.HasOne(x => x.Categoria).WithMany()
                   .HasForeignKey(x => x.CategoriaId)
                   .OnDelete(DeleteBehavior.NoAction);//Para que no nos afecte el borrado en cascada(Si queremos borrar una categoria que este relacionada a productos)
